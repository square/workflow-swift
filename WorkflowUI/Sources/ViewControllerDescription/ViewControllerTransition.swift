 ...
 */
    ...
    /// the `ViewControllerTransition` provided from the `ViewControllerDescription`
    ...
    public extension ViewControllerTransition {
        /// Passed to a `ViewControllerTransition`'s `setup` and `animate`
        ...
        final class Context {
            ...
            public func setCompleted() {
                guard case .running(let running) = state else {
                    fatalError(
                        """
                        WorkflowUI ViewControllerTransition Error: `setCompleted()` was called multiple times to signal \
    ...
    extension ViewControllerTransition.Context {
        private enum State {
