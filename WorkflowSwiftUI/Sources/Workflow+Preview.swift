...
private struct PreviewView<WorkflowType: Workflow>: UIViewControllerRepresentable where WorkflowType.Rendering: Screen {
    ...
    // Both callbacks drive a workflow render pass synchronously, and SwiftUI is what calls them, so
    // every observable read the pass makes is misreported. See `withPerceptionCheckSuppressed` for
    // why, and for why it only happens in previews.
